...
class TestScreenshots implements ScreenshotServiceFactory {
  public create(_browser: Browser, name?: string): ScreenshotService {
    return {
      capture: async (): Promise<Evidence> => ({
        id: name ?? "page",
        name: name ?? "page",
...
class TestReferences implements ReferenceServiceFactory {
  public create(): { retrieve(): Promise<Reference> } {
    return {
      retrieve: async (): Promise<Reference> => ({
        id: "reference",
        name: "reference",
        source: "test",
        localPath: "reference.png",
...
class ControlledComparator implements Comparator {
  ...
  public async compare(
    ...
    return {
      status,
      differencePercentage: status === ComparisonStatus.Passed ? 0 : 5,
      differentPixels: 1,
      totalPixels: 1,
      width: 1,
      height: 1,
      ...
      passed: status === ComparisonStatus.Passed,
      metadata: {},
      duration: 1,
    };
  }
...
const pages: VisualValidationPageConfiguration[] = [
  {
    ...
    reference: { provider: DesignProviderType.Figma, fileKey: "file", nodeId: "node" },
  },
  {
    id: "about",
...
test("evaluates every page and aggregates partial failures", async () => {
  ...
  const plugin = new VisualValidationPlugin(
    ...
    new TestScreenshots(),
    new TestReferences(),
    new ControlledComparator([ComparisonStatus.Passed, ComparisonStatus.Failed]),
    pages,
    comparison
  );
...
test("returns a configuration error for an invalid page without blocking its lifecycle", async () => {
  ...
  const plugin = new VisualValidationPlugin(
    new TestLogger(),
    new TestBrowserFactory(new TestBrowser()),
    new TestScreenshots(),
    new TestReferences(),
    new ControlledComparator([]),
    [invalidPage],
    comparison
  );
  const result = await plugin.execute({ executionId: "test" });
