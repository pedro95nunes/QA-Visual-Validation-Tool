...
class SilentLogger implements Logger { public info(_message: string): void {} public error(_message: string): void {} }
class StaticReferences implements ReferenceServiceFactory {
  public constructor(private readonly reference: Reference) {}
  public create(_request: VisualValidationRequest): ReferenceService { return { retrieve: async () => this.reference }; }
}
...
integrationTest("validates a local page through real Playwright and Pixelmatch", async () => {
  ...
  const browserConfig = { provider: BrowserProvider.Playwright, headless: true, timeout: 30_000, viewport: { width: 800, height: 600 }, url };
  const comparison: ComparisonConfiguration = { comparator: ComparatorProvider.Pixelmatch, pixelThreshold: 0.1, allowedDifferencePercentage: 0, includeAA: false, alpha: 0.5, outputDirectory: join(directory, "diffs") };
  ...
  try {
    ...
    await baseline.launch(); await baseline.open(url);
    await writeFile(referencePath, await baseline.captureScreenshot({ fullPage: true, format: ScreenshotFormat.Png }));
    await baseline.close();
    const reference: Reference = { id: "reference", name: "homepage", source: "local", localPath: referencePath, downloadedAt: new Date(), width: 800, height: 600, metadata: {} };
    const evidenceConfig: EvidenceConfiguration = { outputDirectory: join(directory, "evidence"), fullPage: true, imageFormat: ScreenshotFormat.Png, name: "homepage" };
    const plugin = new VisualValidationPlugin(new SilentLogger(), new DefaultBrowserFactory(browserConfig), new DefaultScreenshotServiceFactory(storage, evidenceConfig), new StaticReferences(reference), new PixelmatchComparator(storage, comparison), [{ id: "homepage", url, reference: { provider: DesignProviderType.Figma, fileKey: "local", nodeId: "local" } }], comparison);
